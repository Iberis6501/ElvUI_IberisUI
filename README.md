...
### v2.28 (2026-07-28)
- 채팅 줄간격 복원을 클래식 에라 1.15.9(빌드 68808+)로 확장 — 에라 신클라도 TBC 68575와 같은 텍스트 렌더링 변경 (20주년 68575는 기존 렌더링이라 미적용)
- 설치 마법사 화면 갱신 보호 — 1.15.9 신클라 script ran too long으로 갱신이 끊겨도 설정 저장은 완료되며 마법사가 계속 진행됨
- 이름표 상하 겹침 해소 — 쌓기 모드(`nameplateMotion` 1) + 세로 간격(`nameplateOverlapV` 1.6) 설치 시 1회 적용 (클래식/불성 계열만, 이후 유저 변경 존중)
