...
### v2.29 (2026-07-28)
- ElvUI 15.18 런타임 핫픽스 내장 — Classic Era 1.15.9(신엔진)에서 ElvUI 15.18 초기화가 붕괴하는 문제를 IberisUI가 게임 안에서 자동 보정. 동봉 애드온 `!IberisFix`(삭제된 전역 선복원) + 런타임 패치 모듈(이벤트 가드·워치독 격벽·블리자드 잔재 청소). 별도 파일 패치 불필요, ElvUI가 신엔진 대응 버전(>15.18)을 내면 자동 비활성
